...
Every row carries its own price, not just its tokens — the comparison is the point. Opus is worth
five times a Sonnet token on output, so a breakdown in tokens alone would hide exactly what someone
opens this for.
...
**`[PURE]` means no engine, no surface, no `# Runway
...
** — data in, data out, and that is where the 72 tests
