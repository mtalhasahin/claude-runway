...
today        740k tokens · 49 turns · $1.69 at API rates

  claude-waitroom        657k
  payra                   83k
...
**`[PURE]` means no engine, no surface, no `$`** — data in, data out, and that is where the 69 tests
...
```bash
claude plugin test .                                # 69 tests, no network, no fs
